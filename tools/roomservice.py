...
if not depsonly:
    print("Device %s not found. Attempting to retrieve device repository from KylinMod Github (http://github.com/KylinMod)." % device)
...
page = 1
while not depsonly:
    githubreq = urllib.request.Request("https://api.github.com/users/KylinMod/repos?per_page=200&page=%d" % page)
    add_auth(githubreq)
...
def add_to_manifest(repositories, fallback_branch = None):
    ...
    for repository in repositories:
        ...
        if exists_in_tree(lm, repo_name):
            print('KylinMod/%s already exists' % (repo_name))
            continue

        print('Adding dependency: KylinMod/%s -> %s' % (repo_name, repo_target))
        project = ElementTree.Element("project", attrib = { "path": repo_target,
            "remote": "kylin", "name": "KylinMod/%s" % repo_name })
...
def fetch_dependencies(repo_path, fallback_branch = None):
    ...
    if os.path.exists(dependencies_path):
        ...
        for dependency in dependencies:
            if not is_in_manifest("KylinMod/%s" % dependency['repository']):
